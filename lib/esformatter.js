...
function format(str, opts){
    ...
    // we trailing whitespace before since it's simpler
    // FIXME: this should probably be applied afterwards and using the AST
    // instead...
...
function processComments(node){
    ...
    while (token && token !== endToken) {
        if (!token._processed && (token.type === 'LineComment' ||
            ...
            _indent.ifNeeded(token, indentLevel);
            _ws.beforeIfNeeded(token, token.type);

            if (token.type === 'BlockComment') {
                var tabIndent = /^\\t+/gm;
                var spaceIndent = /^ +/gm;
                var indent = _indent.getIndent(indentLevel);
                if ( spaceIndent.test(token.raw) ) {
                    token.raw = token.raw.replace(spaceIndent, function(match){
                        return match.length % 2;
                    });
                }
                if ( tabIndent.test(token.raw) ) {
                    // token.raw = 
                }
                // token.raw = token.raw.replace(re, indent);
            }
